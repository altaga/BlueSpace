...
extern const unsigned char previewR[34888];  // Referring to external data (Dingdong audio files are stored inside).   引用外部数据（dingdong音频文件存储在内）
...
void DingDong(void){
  ...
  i2s_write(Speak_I2S_NUMBER, previewR, sizeof(previewR)/sizeof(previewR[0]), &bytes_written, portMAX_DELAY);
}
...
void setup() {
  ...
  M5.Lcd.setCursor(10, 10); // Set the cursor at (10,10).  将光标设在（10，10）处
  M5.Lcd.printf("Speak Test!"); // The screen prints the formatted string and wraps it.  屏幕打印格式化字符串并换行
...
void loop() {
  TouchPoint_t pos= M5.Touch.getPressPoint(); // Stores the touch coordinates in pos.  将触摸坐标存储在pos.内
  if(pos.y > 240)
    if(pos.x < 109)
    {
